...
public:
	...
	FittingAlgorithms(const ReadParseData& data);

	void fitSynclasticShell();
	Eigen::VectorXd getFittedParameters() const;
	std::pair<double, double> getRadiiOfCurvature() const;
	double calculateFitError() const;
	Eigen::Vector3d getDataCenter() const;
	double getDataScale() const;

private:
	//const ReadParseData& readParseData;

	const ReadParseData& m_data;
	Eigen::VectorXd m_fittedParams;
	Eigen::Vector3d m_center;
	double m_scale;

	void centerAndScaleData();
	Eigen::MatrixXd buildDesignMatrix() const;
	Eigen::VectorXd buildTargetVector() const;
};
